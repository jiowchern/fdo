...
// CDXUTIMEEditBox.h
//
#ifndef __CDXUTIMEEditBox_h__
#define __CDXUTIMEEditBox_h__
...
#define MAX_COMPSTRING_SIZE 512
...
protected:
    ...
    static UINT WINAPI Dummy_ImmGetDescription( HKL, LPTSTR, UINT ) { return 0; }
    ...
    static UINT (WINAPI * _ImmGetDescription)( HKL, LPTSTR, UINT );
...
public:
    ...
    void SetIndicatorVisible( const bool &bVisible ) { m_bIndicatorVisible = bVisible; }
    void SetIndicatorImeColor( D3DCOLOR IndicatorBkColor ) { m_IndicatorImeColor = IndicatorBkColor; }
    void OffsetIndicatorImePos( int OffsetX, int OffsetY ) { m_OffsetX = OffsetX; m_OffsetY = OffsetY; }
    void SetShowCompUp( bool bValue ) { m_bShowCompUp = bValue; }
    // ming add <<
    
	// <jiowchern>
	static bool	fnInInputState	() ;
	// </jiowchern>
...
protected:
    ...
    struct CCandList
    {
        WCHAR awszCandidate[MAX_CANDLIST][MAX_COMPSTRING_SIZE];
